...
def main():
    ...
    print("\n" + "=" * 100)
    print("  DYFO NEURO-SYMBOLIC AI & GRAPHRAG LLM PORTFOLIO PERFORMANCE  ")
    print("=" * 100)
    print(f"{'Strategy / Model':<28} | {'Ann. Ret':<9} | {'Ann. Vol':<9} | {'Sharpe':<8} | {'Max DD':<9} | {'Turnover':<9} | {'Wealth'}")
    print("-" * 100)
    for s in strategies:
        res = results_summary[s]
        print(f"{s:<28} | {res['annualized_return']*100:>7.2f}% | {res['annualized_volatility']*100:>7.2f}% | {res['sharpe_ratio']:>8.4f} | {res['max_drawdown']*100:>7.2f}% | {res['turnover']:>9.4f} | {res['final_wealth']:>7.4f}x")
    print("=" * 100 + "\n")
