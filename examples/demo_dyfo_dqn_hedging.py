...
def main():
    ...
    print("\n" + "=" * 115)
    print("  DYFO DISCRETE DEEP Q-NETWORK (DQN) DYNAMIC HEDGING PERFORMANCE (10 BPS TX COST)  ")
    print("=" * 115)
    print(f"{'Strategy / Model':<30} | {'Gross Ret':<9} | {'Net Ret':<9} | {'Net Vol':<9} | {'Net Sharpe':<10} | {'Max DD':<9} | {'Turnover':<9} | {'Cost Drag'}")
    print("-" * 115)
    for m in models:
        res = results_summary[m]
        print(f"{m:<30} | {res['annualized_gross_return']*100:>7.2f}% | {res['annualized_net_return']*100:>7.2f}% | {res['annualized_volatility']*100:>7.2f}% | {res['net_sharpe_ratio']:>10.4f} | {res['max_drawdown']*100:>7.2f}% | {res['turnover']:>9.4f} | {res['annualized_cost_drag_bps']:>6.1f} bps")
    print("=" * 115 + "\n")
